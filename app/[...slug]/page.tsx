...
export async function generateMetadata({params}:{params:Promise<{slug:string[]}>}):Promise<Metadata>{const {slug}=await params;const key=slug.join("/");if(slug.length===1&&isLocale(slug[0])){const lang=slug[0];const t=copy[lang];const canonical=lang==="en"?"/":`/${lang}`;return{title:{absolute:t.metaTitle},description:t.metaDescription,alternates:{canonical,languages:{...Object.fromEntries(locales.map(code=>[code,code==="en"?"/":`/${code}`])),"x-default":"/"}},openGraph:{title:t.metaTitle,description:t.metaDescription,url:`https://qanri.com${canonical}`,locale:lang,type:"website"}}}const item=metadataByRoute[key];return item?{title:{absolute:item.title},description:item.description,alternates:{canonical:`/${key}`}}:{title:"Page not found | QANRI"}}
