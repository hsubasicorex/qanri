...
export async function generateMetadata({params}:{params:Promise<{slug:string[]}>}):Promise<Metadata>{const {slug}=await params;const key=slug.join("/");const item=metadataByRoute[key];return item?{title:{absolute:item.title},description:item.description,alternates:{canonical:`/${key}`}}:{title:"Page not found | QANRI"}}
export default async function CatchAll({params}:{params:Promise<{slug:string[]}>}){const {slug}=await params;return <RoutePage slug={slug.join("/")}/>}
